...
Práce představuje návrh komponentního systému a zdůvodnění vybraných rozhodnutí. Dále je představena implementace v programovacím jazyce \emph{C++}. Nakonec je ukázáno možné využití vytvořené knihovny a návrh hry za použití \emph{datově orientované kompozice}. \\
Vytvořené řešení umožňuje dynamickou kompozici entit za běhu aplikace z komponent definovaných za jejího překladu. Navržený systém je možné začlenit do již existujících projektů, umožňuje paralelní přístup a je uživatelsky rozšiřitelný. \\
Výsledná knihovna umožňuje návrh aplikací za použití \emph{datově orientované kompozice}, bez nutnosti implementace vlastního systému.}
\abstract[en]{The goal of this bachelor thesis is to design and implement a library for management of entities created using \emph{dat-oriented composition}. Analysis of the ways entities are used in todays software is performed -- \emph{object-oriented inheritance}, \emph{object-oriented composition} and \emph{data-oriented composition}. \\
Possible design of such composition system is presented and chosen decisions are discussed. The aforementioned design is then implemented in the \emph{C++} programming language. In the end a demo game implementation is presented using the \emph{data-oriented composition} pattern. \\
Presented solution allows dynamic composition of entities during application runtime from components defined at compile time. System is easily incorporated into existing projects, allows parallel access and can be extended by the user. \\
Resulting library allows experimentation and designing  of applications using \emph{data-oriented composition} pattern, without the need to implement the whole system.}

%Klicova slova (cesky, slovensky ci anglicky) / Keywords (in czech, slovak or english)
\keywords[cs]{Komponentní systém, Entity-component system, ECS, Kompozice, Data-oriented design, Paralelní, C++}
\keywords[en]{Component system, Entity-component system, ECS, Composition, Data-oriented design, Parallel, C++}
